...
while True:
    ...
    searchCommand = regex2.match(command)
    playCommand = regex3.match(command)
    playDirect = regex4.match(command)

    isMediaCommand = True

    if searchCommand or playCommand:
        isMediaCommand = False
    
    if not isMediaCommand:
        # Search command
        if searchCommand:
            # Replace spaces with + to get search page url (a bit crude, will implement better soln later)
            searchquery = searchCommand.group(2).strip().replace(' ', '+')
        ...
        # Specific URL was entered
        elif playDirect:
            url = playDirect.group(2).strip()
            # print ("Direct URL:")
            # print (url)
        ...
        # General Play command - Play the first option in search results
        elif playCommand:
            searchquery = playCommand.group(2).strip().replace(' ', '+')
            url = 'https://youtube.com/results?search_query=' + searchquery
